...
async function getTaskData(taskSlug: string) {
  // Find task by slug (supports debugging alias)
  ...
  // Query prompts by task_id OR task_slug (handles debugging and bug-debugging aliases)
  const filterClause = isDebug 
    ? `task_id.eq.${task.id},task_slug.eq.debugging,task_slug.eq.bug-debugging`
    : `task_id.eq.${task.id},task_slug.eq.${task.slug}`;
