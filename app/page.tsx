...
const MODEL_LOGOS: Record<string, { icon: React.ReactNode; color: string; bg: string }> = {
  chatgpt: {
    color: 'text-[#F58F7C]',
    bg: 'bg-[#F58F7C]/10 border-[#F58F7C]/20',
    icon: (
  ...
  claude: {
    color: 'text-[#F2C4CE]',
    bg: 'bg-[#F2C4CE]/10 border-[#F2C4CE]/20',
    icon: (
      <svg className="w-5 h-5" viewBox="0 0 24 24" fill="currentColor">
...
const ROLE_ICONS: Record<string, { icon: React.ReactNode; color: string }> = {
  developer: { icon: <Code2 className="w-3.5 h-3.5" />, color: 'text-[#F58F7C]' },
  'digital-marketer': { icon: <Megaphone className="w-3.5 h-3.5" />, color: 'text-[#F2C4CE]' },
  founder: { icon: <Rocket className="w-3.5 h-3.5" />, color: 'text-[#F58F7C]' },
  'real-estate-agent': { icon: <HomeIcon className="w-3.5 h-3.5" />, color: 'text-[#F2C4CE]' },
  'seo-specialist': { icon: <SearchIcon className="w-3.5 h-3.5" />, color: 'text-[#D6D6D6]' },
};
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 md:py-12">
      ...
      <section className="text-center py-10 md:py-16">
        <div className="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full bg-[#F58F7C]/15 border border-[#F58F7C]/30 text-[#F58F7C] text-xs font-semibold mb-6">
        ...
        <h1 className="text-3xl sm:text-5xl md:text-6xl font-black text-white tracking-tight leading-tight mb-4">
          ...
          <span className="text-transparent bg-clip-text bg-gradient-to-r from-[#F58F7C] to-[#F2C4CE]">
            For Any Real-World Task
          </span>
        </h1>

        <p className="text-[#D6D6D6] text-sm sm:text-base max-w-2xl mx-auto mb-8 leading-relaxed">
        ...
        {/* Metrics Bar */}
        <div className="grid grid-cols-3 max-w-lg mx-auto mt-10 pt-6 border-t border-[#4F4F51]/60 text-center">
          <div>
            <div className="text-xl sm:text-2xl font-extrabold text-white">{totalPromptsCount}</div>
            <div className="text-[11px] text-[#D6D6D6]/70 font-medium">Total Prompts</div>
          </div>
          <div className="border-x border-[#4F4F51]/60">
            <div className="text-xl sm:text-2xl font-extrabold text-[#F58F7C]">6</div>
            <div className="text-[11px] text-[#D6D6D6]/70 font-medium">AI Models</div>
          </div>
          <div>
            <div className="text-xl sm:text-2xl font-extrabold text-[#F2C4CE]">100%</div>
            <div className="text-[11px] text-[#D6D6D6]/70 font-medium">Quality Audited</div>
          </div>
        </div>
      ...
      {/* EXPLORE BY AI MODEL */}
      <section className="mb-14 border-t border-[#4F4F51]/60 pt-10">
        <div className="flex items-center justify-between mb-5">
          <h2 className="text-lg font-bold text-white flex items-center gap-2">
            <Cpu className="w-4 h-4 text-[#F58F7C]" /> Explore by AI Model
          </h2>
        </div>
          ...
          {modelsList.map((m: any) => {
            ...
            return (
              <Link
                key={m.id || m.slug}
                href={`/models/${m.slug}`}
                className="flex items-center gap-3.5 p-4 rounded-2xl bg-[#4F4F51]/40 hover:bg-[#4F4F51]/80 border border-[#4F4F51] hover:border-[#F58F7C]/50 transition group shadow-md backdrop-blur"
              >
                ...
                <div className="min-w-0">
                  <div className="text-sm font-bold text-white group-hover:text-[#F58F7C] transition-colors truncate">
                    {m.name}
                  </div>
                  <p className="text-xs text-[#D6D6D6]/80 mt-0.5 line-clamp-1">
      ...
      <section className="mb-14">
        <div className="flex items-center gap-2 mb-4">
          <Briefcase className="w-4 h-4 text-[#F2C4CE]" />
          ...
          {professionsList.map((r: any) => {
            ...
            return (
              <Link
                ...
                className="inline-flex items-center gap-2 px-4 py-2.5 rounded-xl bg-[#4F4F51]/40 hover:bg-[#4F4F51]/80 border border-[#4F4F51] text-xs font-semibold text-[#D6D6D6] hover:text-white hover:border-[#F58F7C]/60 transition group shadow-sm"
              >
      ...
      <section className="mb-14">
        <div className="flex items-center justify-between mb-6">
          <div>
            ...
            <p className="text-xs text-[#D6D6D6]/80 mt-0.5">Tested prompts loaded dynamically from Supabase</p>
          </div>
          <Link href="/directory" className="text-xs font-semibold text-[#F58F7C] hover:underline flex items-center gap-1">
        ...
        <section className="mb-14">
          <div className="flex items-center justify-between mb-6">
            <div className="flex items-center gap-2">
              <TrendingUp className="w-5 h-5 text-[#F58F7C]" />
              <div>
                <h2 className="text-xl font-bold text-white">Trending Prompts</h2>
                <p className="text-xs text-[#D6D6D6]/80 mt-0.5">High-engagement workflows this week</p>
              </div>
            </div>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
